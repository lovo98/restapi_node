...
exports.products = async (req, res, next) => {
	try {
		...
		res
			.status(500)
			.json({ mensaje: "Ocurrio un error al consultar productos." });
		next();
	}
};
...
exports.updateProduct = async (req, res, next) => {
	...
	try {
		if (req.file && req.file.filename) {
			dataUpdate.imagen = req.file.filename;
		} else {
			let productOld = await Product.findOne({ where: { id } });
			dataUpdate.imagen = productOld.imagen;
		}
		...
		res
			.status(500)
			.json({ mensaje: "Ocurrio un error al actualizar el producto." });
		next();
	}
};
